...
export async function POST() {
  ...
  const CRM_CONTENT = `<h2>Введение</h2>
...
<p>Эта инструкция охватывает базовые сценарии: вход в систему, создание карточки клиента, работа со сделками, постановка задач и просмотр отчётов. Освоив их, вы сможете самостоятельно вести клиентскую базу.</p>
<h2>Первые шаги после логина</h2>
<p>После входа в систему вы попадаете на главный дашборд. Слева расположено меню с основными разделами: Воронка, Клиенты, Контакты, Задачи, Отчёты. Перед началом работы убедитесь, что в правом верхнем углу выбрана правильная организация и ваша роль соответствует задаче.</p>
<h2>Создание и ведение клиента</h2>
<p>Чтобы добавить нового клиента, нажмите «Создать → Компания». Обязательные поля: название, ИНН, отрасль, ответственный менеджер. Заполните максимум данных сразу — это сэкономит время на последующих этапах.</p>
<p>В карточке компании привяжите контактных лиц и добавьте комментарий с контекстом первого обращения. Если компания уже есть в базе, система предупредит о дубликате.</p>
<h2>Работа с воронкой продаж</h2>
<p>Все сделки движутся по этапам: Новый лид → Квалификация → Потребности → Предложение → Переговоры → Закрыто. Каждый переход должен сопровождаться фиксацией: что сделано, что обсудили, каковы следующие шаги.</p>
<p>Задачи ставятся через кнопку «Новая задача» прямо из карточки сделки — система автоматически привяжет их к клиенту. Так вы никогда не забудете про follow-up.</p>`

  const CLIENT_CONTENT = `<h2>Стандарты коммуникации</h2>
<p>Каждое обращение клиента — это возможность укрепить отношения. Отвечайте на входящие в рамках 2 рабочих часов: даже если полный ответ требует времени, отправьте подтверждение о получении и укажите срок. Уважительное обращение по имени, вежливый тон и чёткие формулировки — обязательные элементы любой переписки и звонка.</p>
<h2>Классификация обращений</h2>
<p>Все обращения делим на 4 категории: консультация (ответ на вопрос), техническая проблема (требует эскалации в поддержку), претензия (передаём руководителю), коммерческий запрос (подключаем отдел продаж). Менеджер обязан корректно классифицировать обращение в CRM в течение 30 минут после получения.</p>
<h2>Эскалация и SLA</h2>
<p>Если клиент недоволен или вопрос выходит за рамки компетенций — передайте задачу руководителю отдела. Критические обращения (проблемы с оплатой, сбои сервиса) решаются в течение 4 часов. Обычные — в течение 1 рабочего дня. Превышение SLA фиксируется и разбирается на еженедельной планёрке.</p>
<h2>Документирование</h2>
<p>Все договорённости с клиентом обязательно отражаются в карточке сделки: дата, суть, следующий шаг. Устные договорённости без записи считаются недействительными. Ежемесячно менеджер анализирует свой портфель клиентов и обновляет статусы.</p>`

  const SAFETY_CONTENT = `<h2>Общие требования</h2>
<p>Перед началом работы на складе каждый сотрудник обязан пройти вводный инструктаж по технике безопасности и расписаться в журнале. Повторный инструктаж проводится раз в 6 месяцев. Без подтверждённого прохождения инструктажа доступ в складские помещения запрещён.</p>
<h2>Работа с грузами</h2>
<p>Подъём и перемещение тяжестей вручную допускается только в пределах установленных норм: до 15 кг для женщин и до 30 кг для мужчин. Для более тяжёлых грузов обязательно использование погрузочной техники или помощь коллег. При работе с грузоподъёмным оборудованием запрещено находиться в зоне движения груза.</p>
<h2>Средства индивидуальной защиты</h2>
<p>На складе обязательно ношение сертифицированной спецобуви с защитным подноском, светоотражающего жилета и каски в зонах, где ведутся погрузочные работы. Перчатки выдаются под роспись и меняются по мере износа. Снятие СИЗ в рабочее время — основание для дисциплинарного взыскания.</p>
<h2>Действия в экстренных ситуациях</h2>
<p>При возгорании немедленно сообщите по внутреннему номеру 01 или используйте кнопку экстренного оповещения. Эвакуация происходит по зелёным указателям к ближайшему выходу. Сбор персонала — на площадке перед главными воротами. При травме вызовите старшего смены и окажите пострадавшему первую помощь.</p>`

  const B2B_SCRIPT_CONTENT = `<h2>Цель холодного звонка</h2>
<p>Холодный звонок не продаёт продукт — он назначает следующую встречу или демо. Реальная задача первого контакта: установить связь с ЛПР (лицом, принимающим решения), выяснить базовую потребность и договориться о следующем шаге. Не пытайтесь «закрыть» сделку в первые 3 минуты разговора.</p>
<h2>Структура разговора</h2>
<p>Приветствие (10 секунд) — представьтесь, назовите компанию и укажите причину звонка одним предложением. Уточнение (15 секунд) — попросите 1 минуту внимания, объясните, почему обращаетесь именно к этому клиенту. Открытый вопрос (1-2 минуты) — узнайте, как клиент сейчас решает проблему, которую закрывает ваш продукт. Предложение встречи (30 секунд) — предложите конкретное время для демонстрации.</p>
<h2>Работа с возражениями</h2>
<p>«Нет времени» — предложите короткую встречу на 15 минут в удобное время. «Нам ничего не нужно» — уточните, почему и предложите альтернативный формат контакта (email с материалами). «Дорого» — не обсуждайте цену по телефону, переведите разговор на ценность и демо.</p>
<h2>После звонка</h2>
<p>Сразу после звонка зафиксируйте в CRM: с кем говорили, что обсудили, какой следующий шаг. Договорились о демо — поставьте задачу на подготовку за день до встречи. Клиент отказался — проставьте причину отказа (это данные для аналитики).</p>`

  const ONBOARDING_CONTENT = `<h2>Первый день</h2>
<p>В первый день нового сотрудника встречает HR-менеджер или его непосредственный руководитель. Новичку выдают бейдж, рабочее место, доступ к корпоративным системам и проводят экскурсию по офису. К полудню сотрудник должен быть представлен команде и получить план работы на первую неделю.</p>
<h2>Первая неделя</h2>
<p>Цель первой недели — погружение в продукт и процессы. Обязательные активности: изучение базы знаний (инструкции, регламенты), знакомство с CRM, просмотр записей клиентских звонков, разбор типовых задач с наставником. В конце недели проводится встреча «Итоги первой недели» с руководителем.</p>
<h2>Первый месяц</h2>
<p>К концу первого месяца сотрудник должен самостоятельно выполнять базовые задачи своей роли, знать имена коллег и понимать процессы взаимодействия с другими отделами. Проводится формальная аттестация: короткий тест по продукту и разбор реальной задачи. Результаты фиксируются в HR-системе.</p>
<h2>Наставничество и обратная связь</h2>
<p>На весь период адаптации (обычно 2-3 месяца) за новичком закрепляется наставник — опытный сотрудник того же отдела. Еженедельные встречи с наставником помогают корректировать траекторию обучения. Руководитель собирает обратную связь от новичка через 2, 4 и 8 недель.</p>`

  const PRIVACY_CONTENT = `<h2>Общие положения</h2>
<p>Настоящая политика описывает принципы обработки персональных данных пользователей нашего сайта и сервисов. Используя сервис, вы соглашаетесь с условиями обработки данных, изложенными ниже. Оператор данных — юридическое лицо, указанное в разделе «Реквизиты».</p>
<h2>Какие данные мы собираем</h2>
<p>Мы обрабатываем: ФИО, контактные данные (email, телефон), данные организации (название, ИНН), техническую информацию об устройстве (IP, User-Agent), историю взаимодействий с сервисом. Сбор производится только при явном действии пользователя: регистрация, заполнение формы, оформление заявки.</p>
<h2>Цели обработки</h2>
<p>Данные используются для предоставления сервиса, коммуникации по заявкам, улучшения продукта и аналитики. Мы не передаём персональные данные третьим лицам без согласия пользователя, за исключением случаев, предусмотренных законодательством РФ.</p>
<h2>Права пользователя</h2>
<p>Пользователь вправе в любой момент запросить доступ к своим данным, уточнить их, удалить или отозвать согласие на обработку. Для этого направьте запрос на privacy@company.ru. Ответ направляется в течение 10 рабочих дней.</p>`

  const FAQ_CONTENT = `<h2>Общие вопросы о продукте</h2>
<p>В этом разделе собраны ответы на самые частые вопросы клиентов о продукте. Материал регулярно обновляется на основе обращений в службу поддержки и отзывов пользователей. Если вы не нашли ответ — напишите нам в поддержку, и мы добавим новый пункт.</p>
<h2>Подписка и оплата</h2>
<p>Подписка активируется сразу после оплаты. Поддерживаемые способы: банковская карта, счёт на юр. лицо, СБП. Для юридических лиц предоставляем закрывающие документы (счёт, акт, счёт-фактура). При отмене подписки доступ сохраняется до конца оплаченного периода.</p>
<h2>Техническая поддержка</h2>
<p>Обратиться в поддержку можно через чат в интерфейсе (кнопка в правом нижнем углу), по email support@company.ru или по телефону в рабочие часы (пн-пт 9:00-18:00 МСК). Среднее время ответа — 15 минут в рабочее время, до 2 часов в выходные.</p>`

  const ONE_C_CONTENT = `<h2>Запуск и вход в систему</h2>
<p>Откройте ярлык 1С на рабочем столе. В появившемся окне выберите информационную базу «Бухгалтерия предприятия» и нажмите «1С:Предприятие». Введите ваш логин и пароль (выдаются системным администратором). После успешного входа вы увидите рабочий стол с основными разделами: Главное, Банк и касса, Покупки, Продажи, Сотрудники.</p>
<h2>Создание документа</h2>
<p>Для создания нового документа перейдите в нужный раздел (например, «Покупки → Поступление») и нажмите «Создать». Заполните обязательные поля: контрагент, договор, дата, сумма. Табличная часть заполняется построчно — каждая строка соответствует одной позиции товара или услуги. После заполнения нажмите «Провести и закрыть».</p>
<h2>Отчёты и выгрузки</h2>
<p>Стандартные отчёты находятся в разделе «Отчёты». Наиболее используемые: Оборотно-сальдовая ведомость, Карточка счёта, Анализ счёта. Период формирования задаётся в верхней части окна. Отчёты можно выгружать в Excel через кнопку «Сохранить как». Регламентированную отчётность формируйте через раздел «Отчёты → Регламентированная отчётность».</p>
<h2>Частые ошибки</h2>
<p>При проведении документа часто возникают ошибки: не заполнен обязательный реквизит, отсутствует счёт учёта, превышен остаток на складе. Внимательно читайте сообщение об ошибке — в нём указано конкретное поле. При любых сомнениях обращайтесь к главному бухгалтеру, не проводите документ наугад.</p>`

  const KB_ARTICLES = [
    { title: "Как работать в CRM",             status: "published", catName: "Инструкции", excerpt: "Пошаговая инструкция по работе с CRM-системой компании.",      content: CRM_CONTENT },
    { title: "Регламент работы с клиентами",   status: "published", catName: "Регламенты", excerpt: "Стандарты коммуникации и качества обслуживания.",              content: CLIENT_CONTENT },
    { title: "Техника безопасности на складе", status: "published", catName: "Регламенты", excerpt: "Обязательные правила безопасности для складских сотрудников.", content: SAFETY_CONTENT },
    { title: "Скрипт холодного звонка B2B",    status: "published", catName: "Продажи",    excerpt: "Готовый скрипт первого контакта с корпоративным клиентом.",    content: B2B_SCRIPT_CONTENT },
    { title: "Онбординг нового сотрудника",    status: "published", catName: "HR",         excerpt: "Программа адаптации на первые 30 дней.",                       content: ONBOARDING_CONTENT },
    { title: "Политика конфиденциальности",    status: "published", catName: "Для сайта",  excerpt: "Обработка персональных данных и коммерческой тайны.",          content: PRIVACY_CONTENT },
    { title: "FAQ по продукту",                status: "draft",     catName: "Продажи",    excerpt: "Ответы на частые вопросы клиентов.",                           content: FAQ_CONTENT },
    { title: "Инструкция по работе с 1С",      status: "review",    catName: "Инструкции", excerpt: "Базовые операции в 1С для новых бухгалтеров.",                 content: ONE_C_CONTENT },
  ]
  ...
  const LEARNING_PLANS_DATA = [
    {
      ...
      // 3 статьи: CRM, Скрипт B2B, Регламент работы с клиентами
      materialRefs: [articleIds[0], articleIds[3], articleIds[1]]
        .filter((id): id is string => !!id)
        .map((id) => ({ materialId: id, materialType: "article" as const })),
      assignees: [sales1Id, sales2Id, salesHeadId, mktId].filter(Boolean),
    },
    {
      title: "Базовое обучение IT",
      description: "Вводный курс для новых разработчиков",
      // 2 статьи: Онбординг нового сотрудника, Политика конфиденциальности
      materialRefs: [articleIds[4], articleIds[5]]
        .filter((id): id is string => !!id)
        .map((id) => ({ materialId: id, materialType: "article" as const })),
    ...
    },
    {
      ...
      // 3 статьи: ТБ на складе, Политика конфиденциальности, Регламент работы с клиентами
      materialRefs: [articleIds[2], articleIds[5], articleIds[1]]
        .filter((id): id is string => !!id)
        .map((id) => ({ materialId: id, materialType: "article" as const })),
      assignees: teamIds, // все 12 сотрудников
    },
  ]
