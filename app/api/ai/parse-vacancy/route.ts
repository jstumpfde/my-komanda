...
- НЕ копируй маркетинговый текст ("Мы ждём амбициозных...", "До встречи в команде") — это НЕ обязанности и НЕ требования
- Обязанности — ТОЛЬКО конкретные рабочие задачи (что делать каждый день)
- Требования — ТОЛЬКО навыки и опыт кандидата (что должен уметь)
- Навыки — краткие фразы: "CRM", "B2B продажи", "Переговоры", "Excel"
- Текст обязанностей и требований — через точку с запятой в ОДНУ строку, НЕ через переносы строк
- Зарплату — только числа, если диапазон "200 000 - 300 000 - 500 000" бери минимум и максимум
- Бонусы — только система оплаты (KPI, процент от продаж), НЕ маркетинг
...
Формат JSON:
{
  ...
  "salaryFrom": "число",
  "salaryTo": "число",
  "bonus": "оклад + KPI + процент от продаж",
  "responsibilities": "задача1; задача2; задача3",
