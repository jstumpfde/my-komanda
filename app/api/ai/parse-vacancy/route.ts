...
conditionsText (Условия — СВОБОДНЫЙ ТЕКСТ, отдельно от тегов):
Что предлагает работодатель, как написано в вакансии: график, оформление, выплаты, соцпакет (ДМС, питание, обучение, парковка, оплата ГСМ, премии…), прочие плюшки. Каждый пункт с новой строки, начинается с —. Если условий в тексте нет — пустая строка. Это ПОЛНЫЙ текст условий (в отличие от conditions — коротких тегов).
...
═══ ФОРМАТ JSON ═══
{
  ...
  "unacceptableSkills": ["стоп-фактор из секции 'Не подойдёт если' или аналог"],
  "conditions": ["ДМС", "Гибкий график"],
  "conditionsText": "условия работы текстом, как в описании (график, оформление, выплаты, соцпакет, оплата ГСМ и т.п.); пустая строка если нет",
  "experienceMin": "число лет или пусто",
  "experienceIdeal": "число лет или пусто",
  "requiredExperience": "none|1-3|3-6|6+ (на основе требуемого опыта: 'от 3 лет' → '3-6', 'без опыта' → 'none')",
  "employmentType": ["Трудовой договор (ТК РФ)"|"ГПХ"|"Самозанятый"|"ИП"],
  "schedule": "5/2|2/2|free|shift|rotation|other (график работы если указан)",
...
function normalize(parsed: Record<string, unknown>) {
  return {
    ...
    requiredExperience: String(parsed.requiredExperience || ""),
    employmentType: toStringArray(parsed.employmentType),
    schedule: String(parsed.schedule || ""),
    employeeType: String(parsed.employeeType || "permanent"),
    hiringPlan: Number(parsed.hiringPlan) || 1,
    conditions: toStringArray(parsed.conditions),
    conditionsText: String(parsed.conditionsText || ""),
