...
export async function GET(request: Request) {
  try {
    ...
    const [
      ...
      // 1. Активные вакансии — всегда «за всё время» (текущее состояние)
      ...
      // 2. Кандидаты по стадиям — воронка «всегда за всё время» (текущее состояние)
      db.select({
      ...
      // 3. По вакансиям: откликов / нанято / интервью / отказов — за период (по candidates.createdAt)
      db.select({
      ...
      // 4. Собеседования — за период (по calendarEvents.createdAt)
      db.select({
      ...
      // 5. Причины отказа — за период (по candidates.createdAt)
      db.select({
      ...
      // 6. Инициатор отказа — за период (по candidates.createdAt)
      db.select({
    ...
    // Сумма всех кандидатов (откликов) — за период из vacancyRows
    const totalCandidates = vacancyRows.reduce((s, r) => s + Number(r.total), 0)

    // Воронка по платформенным стадиям + legacy slugs (живые в prod) — всегда за всё время
    ...
    // ─── KPI ────────────────────────────────────────────────────────
    // hired/rejected — из vacancyTable (за период)
