...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ config: normalizeFunnelV2(desc.funnelV2) })
  } catch (e) {
...
export async function PUT(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const next: FunnelV2Config = normalizeFunnelV2(body.config)
    ...
    const [r] = await db.update(vacancies)
      .set({ descriptionJson: { ...desc, funnelV2: next }, updatedAt: new Date() })
      .where(and(eq(vacancies.id, id), eq(vacancies.companyId, user.companyId)))
