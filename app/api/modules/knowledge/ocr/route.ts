...
// POST /api/modules/knowledge/ocr
//
// Два режима:
//  1) multipart/form-data с полем "file" — распознать изображение/PDF и
//     вернуть извлечённый текст.
//  2) application/json с { text, title, saveToKnowledge: true } — сохранить
//     ранее распознанный текст как черновик статьи.
...
async function callClaudeVision(
  ...
  if (!res.ok) {
    ...
    console.error("[knowledge/ocr] Claude Vision", res.status, errText)
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── Mode A: JSON — save extracted text as article ─────────────────────
    if (contentType.includes("application/json")) {
      ...
      const title = body.title?.trim() || extractTitle(body.text, "Распознанный документ")
    ...
    if (!contentType.includes("multipart/form-data")) {
      return apiError("Используйте multipart/form-data или application/json", 415)
    }
    ...
    try {
      ...
      const title = extractTitle(text, (file as File).name || "Распознанный документ")
