...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (integrations.length === 0) {
      if (run) await finishCronRun(run.id, "ok", { skipped: true })
    ...
    if (run) await finishCronRun(run.id, "ok", { integrations: totalResult.integrations, processed: totalResult.processed, newCandidates: totalResult.newCandidates })
