...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const body = await result.clone().json() as { ok?: boolean; problems?: unknown[] }
    if (run) await finishCronRun(run.id, "ok", { ok: body.ok, problems: Array.isArray(body.problems) ? body.problems.length : 0 })
    return result
  } catch (err) {
