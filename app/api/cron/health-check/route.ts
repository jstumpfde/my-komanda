...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (run) await finishCronRun(run.id, stale ? "error" : "ok", { stale, problemCount: problems.length })
    return NextResponse.json({ ok: !stale, checkedAt: new Date().toISOString(), statuses, problems }, { status: stale ? 503 : 200 })
  } catch (err) {
