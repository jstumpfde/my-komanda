...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const metadata = { tenants: statsByTenant.size, completed: totCompleted, behind: totBehind, overdue: totOverdue, notified }
    if (run) await finishCronRun(run.id, "ok", metadata)
    return NextResponse.json({ ok: true, ...metadata, checkedAt: now.toISOString() })
  } catch (err) {
