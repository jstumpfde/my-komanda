...
interface ApiCompany {
  ...
  crmName: string | null
  salesScripts: string | null
  trainingSystem: string | null
  trainer: string | null
  salesManagerType: string | null
  isMultiProduct: boolean | null
}
    ...
    fetchCompanyApi()
      .then((json) => {
        ...
        const apiCompanyPartial: Partial<Company> = {
          ...
          founded_year: apiData.foundedYear ?? undefined,
          revenue_range: (apiData.revenueRange as Company["revenue_range"]) ?? undefined,
          website: apiData.website ?? undefined,
          crm_status: (apiData.crmStatus as Company["crm_status"]) ?? undefined,
          crm_name: apiData.crmName ?? undefined,
          sales_scripts: (apiData.salesScripts as Company["sales_scripts"]) ?? undefined,
          training_system: (apiData.trainingSystem as Company["training_system"]) ?? undefined,
          trainer: apiData.trainer ?? undefined,
          sales_manager_type: (apiData.salesManagerType as Company["sales_manager_type"]) ?? undefined,
          is_multi_product: apiData.isMultiProduct ?? undefined,
        }
    ...
    if (step === 1) {
      ...
      try {
        await updateCompanyApi({
          ...
          founded_year: draft.company.founded_year,
          revenue_range: draft.company.revenue_range,
          website: draft.company.website?.trim(),
          crm_status: draft.company.crm_status,
          crm_name: draft.company.crm_name?.trim(),
          sales_scripts: draft.company.sales_scripts,
          training_system: draft.company.training_system,
          trainer: draft.company.trainer?.trim(),
          sales_manager_type: draft.company.sales_manager_type,
          is_multi_product: draft.company.is_multi_product,
        })
      } catch {
    ...
    try {
      ...
      try {
        ...
        await updateCompanyApi({
          ...
          crm_name: company.crm_name,
          sales_scripts: company.sales_scripts,
          training_system: company.training_system,
          trainer: company.trainer,
          sales_manager_type: company.sales_manager_type,
          is_multi_product: company.is_multi_product,
        }).catch(() => { /* non-critical */ })
