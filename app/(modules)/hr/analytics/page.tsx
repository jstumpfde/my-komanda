...
import {
  Plus, Copy, Check, Trash2, BarChart3,
  Search, Link2, Download, Share2,
} from "lucide-react"
import {
...
function OverviewTab({ vacancyFilter, onVacancyChange }: { vacancyFilter: string; onVacancyChange: (v: string) => void }) {
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-sm text-muted-foreground/60 mt-1">
          Здесь появится аналитика по воронке найма
        </p>
      </div>
    </div>
  )
...
function AnalyticsContent() {
  ...
  const defaultTab = searchParams.get("tab") === "sources" ? "sources" : "overview"
  ...
  return (
    <Tabs defaultValue={defaultTab}>
      ...
      <TabsContent value="overview">
        <OverviewTab vacancyFilter={vacancyFilter} onVacancyChange={setVacancyFilter} />
      </TabsContent>
        ...
        <main className="flex-1 overflow-auto bg-background">
          <div className="p-4 sm:p-6 max-w-6xl">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h1 className="text-2xl font-semibold">Аналитика HR</h1>
                <p className="text-sm text-muted-foreground">Воронка найма и эффективность источников</p>
              </div>
            </div>
