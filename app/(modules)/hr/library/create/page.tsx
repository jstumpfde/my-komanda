...
function buildPrompt(text: string, params: PromptParams): string {
  ...
  const workFormat = params.workFormat.length > 0
    ...
    : "не указан, используй {{формат_работы}}"
...
Где данные не указаны — используй переменные в двойных фигурных скобках: {{компания}}, {{должность}}, {{город}}, {{зарплата}}, {{формат_работы}}, {{график}}, {{офис}}, {{имя}}. Других переменных не существует — не выдумывай новые, текст с несуществующей переменной уйдёт кандидату с сырыми скобками.
Для фраз с предлогом «в» («работа в …», «офис в …») используй {{в_городе}} вместо «в {{город}}» — она подставится в правильном падеже («в Москве», а не «в Москва»); {{город}} — только для именительного падежа («Город: {{город}}»).
