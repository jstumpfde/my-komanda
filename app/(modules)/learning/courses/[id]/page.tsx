  ...
  const saveProject = async (): Promise<string | null> => {
    ...
    try {
      if (id === "new") {
        ...
        router.replace(`/learning/courses/${created.id}`)
    ...
    <SidebarProvider defaultOpen={true}>
      ...
      <SidebarInset>
        ...
        <div className="flex-1 overflow-auto bg-background min-w-0">
          {/* Header */}
          <div className="border-b px-6 py-4 px-4 sm:px-14">
            <div className="flex items-center gap-1.5 text-sm text-muted-foreground mb-2">
              <Link href="/learning/dashboard" className="hover:text-foreground transition-colors">Обучение</Link>
              <ChevronRight className="size-3.5" />
              <Link href="/learning/courses" className="hover:text-foreground transition-colors">AI-курсы</Link>
              <ChevronRight className="size-3.5" />
