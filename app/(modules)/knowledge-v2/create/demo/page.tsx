...
async function parseWithAI(
  ...
  const res = await fetch("/api/knowledge-v2/parse-with-ai", {
    method: "POST",
    signal,
  ...
  if (!res.ok) {
    ...
    console.error("[parse-with-ai] error", res.status, body)
    throw new Error(`Ошибка обработки: ${res.status}`)
  }

  return await res.json() as ClaudeResult
}
...
export default function CreateDemoPage() {
  ...
  const handleCreateFromDocument = async () => {
    ...
    try {
      ...
      try {
        claudeResult = await parseWithAI(
          extractedText,
          {
            length: docLength,
            tone: docTone,
