...
const nextConfig = {
  ...
  async headers() {
    ...
    return [
      ...
      // Загрузки: SVG под /uploads отдаём как вложение (не рендерим инлайн) —
      // defense-in-depth против stored XSS через ранее загруженные SVG (инлайн-
      // рендер SVG исполняет вложенные скрипты). Новые SVG уже блокируются на
      // загрузке (app/api/upload/*); это подстраховка для файлов, УЖЕ лежащих в
      // /uploads. Ограничено расширением .svg, чтобы не ломать инлайн-просмотр
      // картинок/PDF. Действует только когда статику отдаёт Next; при отдаче
      // через nginx заголовок нужно продублировать в конфиге nginx.
      {
