/**
 * scripts/rescore-axis-preview.ts
 *
 * DRY-RUN превью ОСЕВОГО скоринга резюме (Портрет, редизайн 02.07).
 * Каждый пункт «Подходит» = отдельная ось, оценивается изолированно; веса
 * равные (100/N); итог = среднее осей − штрафы «Не подходит», пол 0.
 * НЕ пишет в БД. Печатает: Имя | было | стало | Δ + разбивку по осям.
 *
 * Запуск (из корня проекта, staging):
 *   set -a; . ./.env.local; set +a
 *   pnpm exec tsx scripts/rescore-axis-preview.ts --limit 30
 */
...
function parseArgs(argv: string[]) {
  let vacancyId = DEFAULT_VACANCY_ID, limit = 30
  const only: string[] = []
  ...
  return { vacancyId, limit, only }
}

interface Row {
  name: string; oldScore: number | null; res: AxisScoreResult | null; error?: string
}
...
async function main() {
  ...
  for (let i = 0; i < uniqueRows.length; i += CONCURRENCY) {
    ...
    const batchResults = await Promise.all(batch.map(async (r): Promise<Row> => {
      ...
      let res: AxisScoreResult | null = null, error: string | undefined
      try {
      ...
      return { name: r.candidateName ?? "—", oldScore: r.resumeScore ?? null, res, error }
    }))
  ...
  console.log(` ${"Имя".padEnd(24)} | было | стало |   Δ   | оси (score→баллы)`)
  console.log("─".repeat(90))
  for (const r of sorted) {
    ...
    console.log(` ${trunc(r.name, 24)} | ${oldS} | ${newS} | ${dStr} | ${r.res.verdict}${weak}`)
