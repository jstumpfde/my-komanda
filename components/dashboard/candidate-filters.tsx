...
// Задача 4 (14.07, корректировка владельца v2): containment-семантика
// «прошёл ДN» — каждый чекбокс требует, чтобы блок был пройден; выбор
// НЕСКОЛЬКИХ = И (кандидат прошёл ВСЕ отмеченные). Про неотмеченные блоки
// ничего не утверждается. Фиксированный набор Д1–Д3 (не читаем реальное
// число демо-блоков вакансии — лишние варианты просто дают 0 совпадений,
// безвредно); покрывает и сегодняшний максимум (2 блока), и «скоро появится
// Демо-3» (владелец, 14.07).
const DEMO_BLOCK_OPTIONS: Array<{ value: string; label: string }> = [
  { value: "1", label: "Прошёл Д1" },
  { value: "2", label: "Прошёл Д2" },
  { value: "3", label: "Прошёл Д3" },
  { value: "none", label: "Не проходил ни одного" },
]
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-4">
          ...
          <Collapsible>
            ...
            <CollapsibleContent className="space-y-3 pt-3">
              ...
              {/* Задача 4 (14.07, корректировка v2): containment «прошёл ДN»
                  (заменяет снятую нотацию «частей: N/M»). Несколько = И. */}
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-muted-foreground">Демо: пройденные блоки (несколько = все выбранные)</label>
                <div className="space-y-1">
