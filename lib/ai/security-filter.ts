...
ВКЛЮЧАЕТ директивный/требовательный тон, если в нём нет оскорблений:
- "Сколько платить именно мне?" → normal (прямой деловой вопрос)
- "скажи зарплату" → normal (короткое деловое требование на ты)
- "когда уже ответ?" → normal (нетерпение по делу)
- "давайте к делу" → normal
...
Примеры: "это странно", "не очень понятно", "не нравится подход", "ну говори же".
Если сомневаешься между normal и mild_negativity — выбирай normal.

medium_abuse — раздражение с грубой лексикой, неуважительный тон, агрессивные претензии без основания.
Примеры: "что за фигня?", "вы там вообще нормальные?", "ой да ладно вы", "вы дурака валяете?".
Маркеры: пренебрежительная грубая лексика («фигня», «дичь», «бред»), нападки на адекватность собеседника без мата.
НЕ medium_abuse: короткий деловой стиль, нетерпеливые требования по теме.

severe_abuse — мат, явные оскорбления личности, угрозы.
Примеры: любой мат (даже завуалированный — «бл*», «х*ня»), "ты идиот", "сволочи", угрозы насилием.
...
кандидат уже проявлял агрессию выше по диалогу — повышай категорию.`
