...
test("filterByStageCondition: «Сегодня» = весь день целиком — идущее прямо сейчас И уже завершившееся сегодня, без дублей в «Прошедшие» (решение владельца 15.07)", () => {
  const now = new Date()
  ...
  // Завершилось ВЧЕРА — должно остаться только в «Прошедшие».
  ...
  // «Прошедшие» — только вчерашнее; сегодняшнее завершившееся туда НЕ дублируется.
  assert.deepEqual(filterByStageCondition(list, "date_before").map(x => x.id), ["endedYesterday"])
})
