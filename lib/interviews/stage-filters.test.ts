...
function iv(id: string, candidateId: string | null, dateStr: string, status = "Подтверждено", interviewDecision: InterviewDecision = null, endStr?: string, candidateStage: string | null = null): FilterableInterview {
  return { id, candidateId, date: new Date(dateStr), endAt: endStr ? new Date(endStr) : undefined, status, interviewDecision, candidateStage }
}
...
test("filterByStageCondition: outcome_rejected пропускает только interviewDecision=reject (гибридные табы 15.07)", () => {
  const list = [
    iv("a", "cand-1", "2026-07-01T10:00:00", "Подтверждено", "reject"),
    iv("b", "cand-2", "2026-07-01T10:00:00", "Подтверждено", "advance"),
    iv("c", "cand-3", "2026-07-01T10:00:00", "Подтверждено", "reserve"),
    iv("d", "cand-4", "2026-07-01T10:00:00", "Подтверждено", null),
  ]
  const result = filterByStageCondition(list, "outcome_rejected")
  assert.deepEqual(result.map(x => x.id), ["a"])
})

test("filterByStageCondition: stage_decision пропускает только кандидатов на стадии воронки «decision» (гибридные табы 15.07)", () => {
  const list = [
    iv("a", "cand-1", "2026-07-01T10:00:00", "Подтверждено", null, undefined, "decision"),
    iv("b", "cand-2", "2026-07-01T10:00:00", "Подтверждено", null, undefined, "reference_check"),
    iv("c", "cand-3", "2026-07-01T10:00:00", "Подтверждено", null, undefined, null),
  ]
  const result = filterByStageCondition(list, "stage_decision")
  assert.deepEqual(result.map(x => x.id), ["a"])
})
...
test("filterByStageCondition: «Сегодня» = весь день целиком; завершившееся сегодня НАМЕРЕННО дублируется в «Прошедшие» (решение владельца 15.07)", () => {
  const now = new Date()
  ...
  // Завершилось СЕГОДНЯ: началось 2ч назад, закончилось час назад.
  const endedToday = iv("endedToday", "c2", new Date(now.getTime() - 2 * 3600_000).toISOString(), "Пройдено", null, new Date(now.getTime() - 3600_000).toISOString())
  // Завершилось ВЧЕРА — только в «Прошедшие».
  const yesterday = new Date(now); yesterday.setDate(yesterday.getDate() - 1)
  const endedYesterday = iv("endedYesterday", "c3", yesterday.toISOString(), "Пройдено", null, yesterday.toISOString())
  const list = [ongoing, endedToday, endedYesterday]
  // «Сегодня» — и идущее сейчас, и уже завершившееся сегодня; вчерашнее не входит.
  assert.deepEqual(filterByStageCondition(list, "date_today").map(x => x.id).sort(), ["endedToday", "ongoing"])
  // «Прошедшие» — вчерашнее И сегодняшнее завершившееся (дубль с «Сегодня»
  // намеренный). Идущее прямо сейчас (endAt в будущем) сюда НЕ попадает.
  assert.deepEqual(filterByStageCondition(list, "date_before").map(x => x.id).sort(), ["endedToday", "endedYesterday"])
})
