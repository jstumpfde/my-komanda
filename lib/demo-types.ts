...
// ─── Material type (тип материала библиотеки) ────────────────────────────────
// Этап 1: отдельной колонки type в demo_templates НЕТ — тип выводится из length.
// length='block' = переиспользуемый «Блок компании» (см. seed-company-blocks),
// иначе обычная демонстрация. 'anketa'/'test' зарезервированы под Этапы 2-3
// (свои источники данных / вкладки), сейчас из demo_templates не выводятся.
...
export const MATERIAL_TYPE_LABELS: Record<MaterialType, { label: string; cls: string }> = {
  demo:   { label: "Демо",   cls: "bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-400" },
  block:  { label: "Блок",   cls: "bg-purple-100 text-purple-700 dark:bg-purple-950/40 dark:text-purple-400" },
  anketa: { label: "Анкета", cls: "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400" },
  test:   { label: "Тест",   cls: "bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-400" },
}
