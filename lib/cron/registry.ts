...
export const CRON_REGISTRY: CronEntry[] = [
  ...
  },
  {
    ...
    schedule:    "* * * * *",
    description: "Импорт откликов с hh.ru и запуск очереди AI-обработки (каждую минуту)",
  },
  {
  ...
  },
  {
    ...
    schedule:    null,
    description: "Проверяет актуальность материалов базы знаний, помечает устаревшие, уведомляет директора",
  },
  {
    name:        "knowledge-gaps",
    endpoint:    "/api/cron/knowledge-gaps",
    schedule:    null,
    description: "Еженедельный анализ неотвеченных вопросов базы знаний, AI-рекомендации что создать",
  },
  {
    name:        "knowledge-progress",
    endpoint:    "/api/cron/knowledge-progress",
    schedule:    null,
    description: "Еженедельная сводка по прогрессу обучения сотрудников (завершили/отстают/просрочили)",
  },
  {
    name:        "knowledge-reminders",
    endpoint:    "/api/cron/knowledge-reminders",
    schedule:    null,
    description: "Ежедневные напоминания сотрудникам с дедлайном по курсу < 3 дней",
  },
  {
    name:        "knowledge-review",
    endpoint:    "/api/cron/knowledge-review",
    schedule:    null,
    description: "Помечает материалы базы знаний, требующие ревью (просроченные или с устаревшим циклом)",
  },
  {
  ...
  },
  {
    ...
    schedule:    "*/15 * * * *",
    description: "Напоминания кандидатам о предквалификации, fallback-финализация по таймауту",
  },
  {
    name:        "pulse-alerts",
    endpoint:    "/api/cron/pulse-alerts",
    schedule:    null,
    description: "Создаёт уведомления для сотрудников с низким пульс-баллом (< 3/5) за 7 дней",
  },
  {
    name:        "recalculate-flight-risk",
    endpoint:    "/api/cron/recalculate-flight-risk",
    schedule:    null,
    description: "Пересчитывает баллы риска увольнения для всех сотрудников по доступным данным",
  },
  {
    name:        "recalculate-integrator-levels",
    endpoint:    "/api/cron/recalculate-integrator-levels",
    schedule:    null,
    description: "Пересчитывает уровни интеграторов по количеству привязанных клиентов",
  },
  {
    name:        "rubric-score",
    endpoint:    "/api/cron/rubric-score",
    schedule:    "*/15 * * * *",
    description: "Автоматический AI-скоринг кандидатов по рубрике вакансии (рубрика-оценка)",
  },
  {
    name:        "sales-follow-up",
    endpoint:    "/api/cron/sales-follow-up",
    schedule:    "*/15 * * * *",
    description: "Дожим лидов продаж: шаблонные касания для активных диалогов без брони",
  },
  {
    name:        "trash-cleanup",
    endpoint:    "/api/cron/trash-cleanup",
    schedule:    "0 0 * * *",
    description: "Авто-удаление вакансий из корзины по истечении срока хранения компании (00:00 UTC)",
  },
  {
    name:        "yandex-direct-agent",
    endpoint:    "/api/cron/yandex-direct-agent",
    schedule:    "0 3,9,15,21 * * *",
    description: "AI-агент Яндекс.Директа: синк кампаний/статистики + оптимизация (4 раза в сутки)",
  },
]
