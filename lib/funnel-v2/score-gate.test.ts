...
test("normalizeScoreGate: undefined/битое → undefined (гейт отсутствует)", () => {
  ...
  // Объект без scoreType → гейт с дефолтами (документированный контракт:
  // «дефолты только для заданного объекта»; autoEnabled=false → поведение не
  // меняется). Стейл-ожидание выровнено с давно действующим кодом.
  assert.deepEqual(normalizeScoreGate({ threshold: 50 }),
    { scoreType: "resume", threshold: 50, failAction: "preliminary_reject", autoEnabled: false })
})
...
test("normalizeScoreGate: неизвестный failAction → дефолт 'preliminary_reject'; autoEnabled по умолчанию false", () => {
  // Дефолт failAction в коде — 'preliminary_reject' (обратимый), не 'manual'
  // (стейл-ожидание выровнено с давно действующим кодом).
  const g = normalizeScoreGate({ scoreType: "test", threshold: 70, failAction: "bogus" })
  assert.equal(g?.failAction, "preliminary_reject")
  assert.equal(g?.autoEnabled, false) // не задан → выкл (ничего не меняется)
})

test("normalizeScoreGate: threshold клампится в 0..100; NaN → дефолт 50", () => {
  ...
  // NaN не проходит Number.isFinite → дефолт DEFAULT_SCORE_GATE_THRESHOLD=50
  // (стейл-ожидание 0 выровнено с давно действующим кодом).
  assert.equal(normalizeScoreGate({ scoreType: "resume", threshold: NaN, autoEnabled: true })?.threshold, 50)
})
