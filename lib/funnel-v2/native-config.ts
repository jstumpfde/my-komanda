 ...
 *
 * Принцип (обновлено 14.07 после предеплой-гарда, находки 1-3): движок
 * выключен ИЛИ соответствующий нативный блок ещё не сохранён (config.stage1/
 * stage2/communications == null) → эффективные значения берутся из Портрета
 * (spec), а не из платформенных дефолтов — prefillNativeFromSpec копирует
 * Портрет в нативные поля при первом открытии конструктора, но полагаться на
 * то, что это УЖЕ произошло, небезопасно (движок можно включить до первого
 * открытия панели стадии). Только когда движок включён И блок реально
 * сохранён — используются нативные значения. Все резолверы
 * (resolveStage1/resolveEffectiveAnketaPassInvite/resolveTgAlerts/
 * resolveHotCandidate) симметричны этому принципу.
 *
