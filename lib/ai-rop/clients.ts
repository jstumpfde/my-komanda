 ...
 *
 * Порт call-agent lib/clients.ts. Отличия: normalizePhone переиспользуем из
 * lib/ai-rop/pii-mask (единая нормализация телефонов по всему модулю, вместо
 * дублирующей локальной реализации оригинала); прод только PG — regexp_replace
 * доступен напрямую, SQLite-фолбэк (LIKE по неполной нормализации) не нужен.
 */
 ...
 * Группировка по нормализованному телефону (в JS — постфильтр, т.к. в БД
 * телефоны хранятся в разных форматах).
 */
export async function listClients(opts: {
  ...
  if (opts.managerId) conditions.push(sql`c.manager_id = ${opts.managerId}`)
  if (opts.search) {
    const s = `%${opts.search}%`
    conditions.push(sql`(c.client_phone ILIKE ${s} OR c.manager_name ILIKE ${s} OR a.client_name ILIKE ${s})`)
  }
  ...
  const rows = (await db.execute(sql`
    SELECT
      ...
      MAX(c.started_at)::text AS last_at,
      ...
      SUM(CASE WHEN a.next_action IS NULL OR a.next_action = '' OR a.next_action = '—' THEN 1 ELSE 0 END)::int AS no_next_action
    FROM rop_calls c
...
export async function getClientProfile(opts: {
  ...
  const conditions: SQL[] = [sql`c.tenant_id = ${opts.tenantId}::uuid`]
  ...
  const variantConds = variants.map((v) => sql`c.client_phone LIKE ${"%" + v + "%"}`)
  conditions.push(sql`(${sql.join(variantConds, sql` OR `)})`)
  ...
  const rows = (await db.execute(sql`
    SELECT
      c.id AS id, c.interaction_type AS interaction_type, c.channel AS channel, c.direction AS direction,
      c.started_at::text AS started_at, c.duration_sec AS duration_sec, c.status AS status,
      c.manager_name AS manager_name, c.manager_id AS manager_id, c.client_phone AS client_phone,
      c.bitrix_deal_id AS bitrix_deal_id, c.bitrix_lead_id AS bitrix_lead_id, c.bitrix_contact_id AS bitrix_contact_id,
      a.summary AS summary, a.sentiment AS sentiment, a.manager_score AS manager_score,
      a.next_action AS next_action, a.client_name AS client_name
    FROM rop_calls c
    LEFT JOIN rop_analyses a ON a.call_id = c.id
    WHERE ${where}
    ORDER BY c.started_at DESC NULLS LAST, c.id DESC
    LIMIT 500
  `)) as unknown as Array<InteractionTimelineItem & { client_phone: string; client_name: string | null }>
