/**
 * Забор email и чатов (Open Lines) из Bitrix24 (порт call-agent lib/bitrix-activities.ts).
 *
 ...
 * TYPE_ID:
 *   1 — встреча
 *   2 — звонок (забирается отдельно, через voximplant.statistic.get)
 *   3 — задача
 *   4 — email
...
function cleanBody(raw: string, descriptionType: string): string {
  ...
  if (descriptionType === "3") {
    text = text
      .replace(
        /\[\/?(b|i|u|url[^\]]*|color[^\]]*|size[^\]]*|font[^\]]*|center|left|right|justify|s|sub|sup|quote[^\]]*|code|spoiler[^\]]*|list|\*|table|tr|td|img[^\]]*|disk[^\]]*)\]/gi,
        ""
      )
      .replace(/\[br\]/gi, "\n");
  }
...
function normalizeActivity(raw: BitrixActivityRaw, tenantId: number): NormalizedInteraction | null {
  ...
  const contentText = subject && body ? `Тема: ${subject}\n\n${body}` : body || subject;
  ...
  return {
    ...
    managerName: null, // подтянется через managers cache (фаза 2b)
...
/**
 * Тянет встречи, email и Open Lines чаты из Bitrix24. Идемпотентно — повторный
 * запуск не создаст дублей (дедуп — внутри saveInteraction, фаза 2b).
 *
 * Фильтры:
 ...
 *
 * Лимит на прогон — пагинация через start. Если activities в Bitrix больше
 * limit — берём первые limit; в следующий вызов с since=last_created
 * подтянутся следующие (метку since хранит вызывающий пайплайн, фаза 2b).
 */
export async function fetchEmailAndChats(
  client: BitrixClient,
  opts: FetchOptions,
  saveInteraction: SaveInteractionFn
): Promise<FetchResult> {
  ...
  for (const typeId of ["1", "4", "6"]) {
    while (processed < limit) {
      ...
      // Для TYPE_ID=6 Bitrix не умеет LIKE по PROVIDER_ID в filter — фильтруем
      // в normalizeActivity (PROVIDER_ID.startsWith('IMOL_')).

      const page = await client.call<BitrixActivityRaw[]>("crm.activity.list", {
        filter,
        order: { CREATED: "DESC" },
        select: [
          "ID",
      ...
      for (const raw of page) {
        ...
        try {
          ...
          console.warn(`[ai-rop/bitrix-activities] normalize/save failed for activity ${raw.ID}:`, (e as Error).message);
          result.errors += 1;
        }
