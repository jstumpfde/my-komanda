 ...
 *
 * Текст формируется в BBCode ([B], [URL], переносы строк) — рассчитан на
 * отправку через lib/ai-rop/bitrix-im.ts imSendMessage (фаза 2a, уже
 * перенесена параллельным агентом).
 *
 ...
 *
 * Порт call-agent lib/reports.ts 1:1 (tenantId → uuid, ссылка на дашборд —
 * платформенный маршрут модуля вместо call-agent URL).
 */
...
// Ссылка «Подробнее» в отчёте — платформенный маршрут модуля AI-РОП
// (UI дашборда — отдельная фаза, /ai-rop уже зарезервирован планом).
const DASHBOARD_URL = "https://company24.pro/ai-rop"
