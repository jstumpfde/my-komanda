  ...
  properties: {
    ...
    coaching_tips: {
      ...
      description:
        ...
        "Эти советы читает сам менеджер, поэтому тон уважительный и помогающий, НИКОГДА не обвиняющий. Правила:\n" +
    ...
    rop_notes: {
      ...
      items: { type: "string" },
      description:
        "1–3 ПРЯМЫХ наблюдения для РОПа (руководителя отдела продаж). Менеджер их НЕ видит — " +
        "поэтому здесь без смягчения: что реально происходит, где менеджер системно теряет, стоит ли вмешаться. Правила:\n" +
        "1) По фактам и прямо, но профессионально — это управленческая заметка, НЕ оскорбление. Не «дурак/ленивый», " +
        "а «не отрабатывает возражение по цене — теряет на этом сделки; повторяется на 3-м звонке подряд».\n" +
        "2) Фокус на том, что важно руководителю: риск для сделки/выручки, системность проблемы, нужен ли разбор 1-на-1, " +
        "сильные стороны (кого можно ставить в пример).\n" +
        "3) Если звонок штатный и вмешательство не требуется — так и напиши коротко («работает по скрипту, вмешательство не нужно») " +
        "или оставь пустой массив [].\n" +
        "4) Не дублируй дословно coaching_tips — там мягкая версия для менеджера, здесь суть для управленца. " +
