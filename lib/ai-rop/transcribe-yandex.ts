/**
 * Транскрипция через Yandex SpeechKit STT v3 (async, «длинное аудио») —
 * порт call-agent lib/transcribe-yandex.ts. Основной канал распознавания
 * (152-ФЗ — эндпоинт в РФ, без прокси, родной русский, биллинг ₽).
 *
 ...
 *
 * БЫЛО: ключ читался из env YANDEX_STT_API_KEY напрямую. СТАЛО: sttConfig
 * (RopSttConfig из rop_settings) — параметр, каждая компания может задать свой
 * ключ/фолдер. Учёт расхода (recordUsage) — колбэк, БД-слой в фазе 2b.
 */
import fs from "fs";
import type { RopSttConfig, RopUsageHooks } from "./types";
...
export async function transcribeYandex(
  ...
  opts: { tenantId?: number; callId?: number; usage?: RopUsageHooks } = {}
): Promise<TranscribeResult> {
  const key = sttConfig.yandexApiKey?.trim();
  if (!key) throw new Error("RopSttConfig.yandexApiKey не задан");
  ...
  // recordUsage — опциональный колбэк, реальная запись в rop_usage_events в фазе 2b.
  if (opts.tenantId && opts.usage?.recordUsage) {
    ...
    try {
      await opts.usage.recordUsage("yandex_stt_seconds", seconds, opts.callId);
    } catch {
      /* учёт не критичен */
    }
  }
