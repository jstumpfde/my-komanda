 ...
 *
 * Данные: rop_analyses.checklist_scores_json ([{id,title,score 0..1,notes?}])
 * + rop_calls (manager_id, started_at). Порт call-agent lib/team-dynamics.ts
 * 1:1 — сохранены важные фиксы оригинала:
 *  - сравнение ранней/недавней половины по ОБЩИМ пунктам чек-листа (иначе
 *    расширение чек-листа со временем ложно занижает недавнюю половину);
 *  - исключение «нерелевантных» пунктов (аналитик ставит им score=1 с
 *    пометкой в notes — иначе они ложно завышают короткие no-contact звонки);
 *  - порог ≥10 звонков на менеджера, чтобы не строить вердикт по шуму.
 */
...
interface CallRow { manager_id: string | null; manager_name: string | null; started_at: string; checklist_scores_json: unknown }
...
export async function getTeamDynamics(tenantId: string, weeks = 8): Promise<ManagerDynamics[]> {
  ...
  const rows = (await db.execute(sql`
    SELECT c.manager_id AS manager_id, c.manager_name AS manager_name,
           c.started_at::text AS started_at, a.checklist_scores_json AS checklist_scores_json
    FROM rop_analyses a
    JOIN rop_calls c ON c.id = a.call_id
    WHERE c.tenant_id = ${tenantId}::uuid
      AND a.checklist_scores_json IS NOT NULL
      AND jsonb_array_length(a.checklist_scores_json) > 0
      AND substr(c.started_at::text, 1, 10) >= ${fromStr}
      AND substr(c.started_at::text, 1, 10) <= ${toStr}
  `)) as unknown as CallRow[]

  // Имена менеджеров (справочник rop_managers.bitrix_manager_id → name; но приоритет — rop_calls.manager_name)
  const nameRows = (await db.execute(sql`
    SELECT bitrix_manager_id AS id, name AS name FROM rop_managers WHERE tenant_id = ${tenantId}::uuid
  `).catch(() => [])) as unknown as Array<{ id: string; name: string | null }>
  ...
  const exclRows = (await db.execute(sql`
    SELECT bitrix_manager_id AS id FROM rop_managers WHERE tenant_id = ${tenantId}::uuid AND excluded_from_reports = true
  ...
  for (const r of rows) {
    ...
    const items = Array.isArray(r.checklist_scores_json) ? (r.checklist_scores_json as ScoreItem[]) : []
